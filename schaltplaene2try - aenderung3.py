...
for s, (root, dirs, files) in enumerate(os.walk(pathsp)):
    if "UV" in root or "Lüftung" in root or "NSHV" in root or "Serverraum 2" in root or "Halle" in root or "Technikum" in root:
        if "Archiv" in root:
            ...
            print(counter, " Datei/en sind vorhanden!")
            counter = 0
            sp = False
            prtk = False
            leg = False
...
for s, (root, dirs, files) in enumerate(os.walk(pathsp)):
    if "UV" in root or "Lüftung" in root or "NSHV" in root or "Serverraum 2" in root or "Halle" in root or "Technikum" in root:
        if "Archiv" in root:
            ...
            counterfolder += 1
            print(f"Ordner {counterfolder} ({seclast}\\{lastpart}) enthält:")#root
            lastpart = os.path.basename(os.path.normpath(root))
            splitin2 = lastpart.split(".")
            beforedot = splitin2[0].replace("UV ", "")
            afterdot = splitin2[-1]
            ...
            #thirdlast = os.path.split(os.path.split(os.path.split(root)[0])[1])
            ...
            for file in files:
                if "Schaltplan" in file and file.endswith(".pdf"):
                    ...
                    print("Dateiname eventuell anpassen!")
